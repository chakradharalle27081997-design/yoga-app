...
export function buildPrompt({ client, session, forOllama = false, cycleNumber = 1, previousPoses = [], asanaCount = null }) {
  ...
  const hasSevereCondition = [...clientConditions, ...clientInjuries].some(c => severeConditions.includes(c));
  const includeSurya = !hasSevereCondition && (clientLevel !== "beginner" || cycleNumber >= 2);
  const suryaInstruction = includeSurya
    ? "MANDATORY: Include Surya Namaskar (Sun Salutation) as the first or second asana. It is required for this student."
    : clientLevel === "beginner" && cycleNumber === 1
    ? "Do NOT include Surya Namaskar — student is in cycle 1, not ready yet."
    : "Do NOT include Surya Namaskar — student has a severe medical condition that contraindicates it.";
...
SURYA NAMASKAR RULE: ${suryaInstruction}
IMPORTANT: NEVER include Surya Namaskar or Sun Salutation in the Warm-up phase. It must ONLY appear as a separate instruction, never as a warm-up pose.
${previousPosesText}
