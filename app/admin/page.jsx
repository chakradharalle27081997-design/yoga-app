...
export default function AdminPage() {
  ...
  if (!authed) return (
    <div style={{ minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", background: "#f9f5f0" }}>
      <div style={{ padding: "40px", background: "white", borderRadius: "16px", boxShadow: "0 4px 20px rgba(0,0,0,0.1)", maxWidth: "360px", width: "100%" }}>
        <h2 style={{ color: "#3E4A2A", textAlign: "center" }}>🔐 Navira Yoga Admin</h2>
        ...
        <button onClick={login} style={{ width: "100%", padding: "14px", background: "#3E4A2A", color: "white", border: "none", borderRadius: "8px", marginTop: "16px", fontSize: "16px", cursor: "pointer" }}>
          Login
        </button>
      </div>
    </div>
  ...
  return (
    <div style={{ padding: "32px", background: "#f9f5f0", minHeight: "100vh" }}>
      <h1 style={{ color: "#3E4A2A" }}>🧘 Navira Yoga — Studio Management</h1>
      <table style={{ width: "100%", borderCollapse: "collapse", background: "white", borderRadius: "12px", overflow: "hidden", boxShadow: "0 2px 10px rgba(0,0,0,0.08)", marginTop: "24px" }}>
        <thead>
          <tr style={{ background: "#3E4A2A", color: "white" }}>
            ...
            <tr key={s.id} style={{ borderBottom: "1px solid #eee", background: i % 2 === 0 ? "white" : "#f9f9f9" }}>
              ...
              <td style={{ padding: "12px" }}>
                <span style={{ padding: "4px 10px", borderRadius: "20px", fontSize: "13px",
                  background: s.status === "approved" ? "#E7ECDA" : s.status === "pending" ? "#fff3cd" : "#f8d7da",
                  color: s.status === "approved" ? "#202A15" : s.status === "pending" ? "#856404" : "#721c24" }}>
                  {s.status}
                </span>
              </td>
              <td style={{ padding: "12px", display: "flex", gap: "8px" }}>
                {s.status !== "approved" && (
                  <button onClick={() => updateStatus(s.id, "approved")}
                    style={{ padding: "6px 12px", background: "#3E4A2A", color: "white", border: "none", borderRadius: "6px", cursor: "pointer" }}>
                    Approve
                  </button>
                )}
                {s.status === "approved" && (
