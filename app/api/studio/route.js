...
export async function PUT(req) {
  try {
    ...
    if (studio.status === "pending") {
      return NextResponse.json({ error: "Your account is pending approval from Navira Yoga Studio" }, { status: 403 });
    }
    if (studio.status === "suspended") {
      return NextResponse.json({ error: "Your account has been suspended. Contact Navira Yoga Studio." }, { status: 403 });
    }
    return NextResponse.json({ studio });
  } catch (err) {
    return NextResponse.json({ error: err.message }, { status: 500 });
